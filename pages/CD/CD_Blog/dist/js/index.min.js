...
function ct_set__category() {
  var ct_category__container = document.querySelector('#ct_blog__article .ct_category__container');
  // let ct_category = ct_category__container.querySelector('h2').innerHTML.toLocaleLowerCase().trim().replace(/\s/g, '').replace(/[&\/\\#,+()$~%.'":*?<>{}]/g, '');
  var ct_category = ct_category__container.querySelector('h2').getAttribute('data-category-icon');
  ct_category__container.href = ct_category__container.href + "#".concat(ct_category);
  ct_category__container.querySelector('svg').innerHTML = "<use xlink:href=\"#CD_".concat(ct_category, "\"></use>");
}
function ct_build__jsonld() {
  var ct_struc_data = {
    ...
    "mainEntityOfPage": {
      "@type": "WebPage",
      "id": "".concat(location.href)
    },
    "headline": "".concat(document.querySelector('#ct_article__title').innerHTML.trim()),
    "url": "".concat(location.href, "}"),
    "datePublished": "".concat(document.querySelector('.ct_pub__date time').getAttribute('datetime')),
    "dateModified": "".concat(document.querySelector('.ct_pub__date time').getAttribute('datetime')),
    "author": {
      "@type": "Person",
    ...
    "description": "".concat(document.querySelector('.ct_article_content p').innerHTML.trim())
  };
