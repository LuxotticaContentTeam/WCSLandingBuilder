...
function ct_wow__search__start_retrySelector(selector,section){
  window.ct_wow__search.initTimer = setTimeout(function(){
      if(document.querySelector(selector)){
        ...
        // console.log('SGH - Selector not found start searching for: '+ selector);
...
if ($('.sgh-search-suggestions__wrapper ').length > 0 && $('#ct_wow__quiz').length == 0){
  ...
  var ct_wow__search__interval_timer = 0;
  const ct_wow__search__interval = setInterval(function(){
    if ($('.sgh-search-suggestions__wrapper ').length > 0 && $('#ct_wow__quiz').length == 0){
      $('.sgh-search-suggestions__wrapper ').after(`
      <div id="ct_wow__quiz">
        <h3>Can’t decide?</h3>
      ...
      clearInterval(ct_wow__search__interval)
    }else{
      ct_wow__search__interval_timer += 1000;
      if (ct_wow__search__interval_timer > 10000){
        clearInterval(ct_wow__search__interval)
      }
    }
  },1000)
