...
window.ct_wow__search.structure = {
      ...
      return _regenerator["default"].wrap(function _callee$(_context) {
        while (1) switch (_context.prev = _context.next) {
          case 0:
            ...
            _context.next = 7;
            return (0, _utils.checkData)(window.ct_wow__search.data, 3000);
          case 7:
            if (!(window.ct_wow__search.data.loaded === false)) {
              _context.next = 10;
              break;
            }
            (0, _utils.customLog)('WOW SEARCH INIT - TIMEOUT');
          ...
          case 10:
            ...
            this.container.addEventListener('keyup', function (e) {
              if ((e.keyCode === 9 || e.which === 9) && _this.accessibility_mode === false) {
                _accessibility.accessibility.init(true);
              }
            });
  ...
  setPlaceholders: function setPlaceholders(first) {
    ...
    var setPlaceholderCircle = function setPlaceholderCircle(first) {
      var circles = Object.keys(_this3.placeholders.utils);
      var comulativePos = 0;
      ...
      circles.forEach(function (circleID, circleN) {
        var container = _this3.placeholders.utils[circleID].container = document.querySelector("#ct_wow__search .ct_wow__search__pos_placeholders__".concat(circleID, " "));
        if (first) {
          // inserting placeholder in the HTML just the first time
          for (var i = 0; i < _this3.placeholders.utils[circleID].prodsCount; i++) {
         ...
         */
        _this3.placeholders.utils[circleID].radius = container.clientWidth / 2;
        var coordinates = (0, _utils.calcCoordinates)(_this3.placeholders.utils[circleID].prodsCount, _this3.placeholders.utils[circleID].radius);
        container.querySelectorAll('div').forEach(function (elem, i) {
           ...
           */
          if (first) {
            _this3.placeholders.coordinates.push({
              elem: elem,
              x: elem.getBoundingClientRect().x + elem.clientWidth / 2,
              y: elem.getBoundingClientRect().y + elem.clientHeight / 2,
            ...
            _this3.placeholders.coordinates[i + comulativePos].x = _this3.placeholders.coordinates[i + comulativePos].elem.getBoundingClientRect().x + _this3.placeholders.coordinates[i + comulativePos].elem.clientWidth / 2;
            _this3.placeholders.coordinates[i + comulativePos].y = _this3.placeholders.coordinates[i + comulativePos].elem.getBoundingClientRect().y + _this3.placeholders.coordinates[i + comulativePos].elem.clientHeight / 2;
          }
        });
        comulativePos += _this3.placeholders.utils[circleID].prodsCount;
      });
    };
  ...
  // },
  rankingProducts: function rankingProducts(qIndex, aIndex, aInnerText) {
    var _this4 = this;
    ...
    this.prod_list.forEach(function (prod) {
      prod.score = _this4.calcScore(prod.upc);
    });
    ...
    this.prod_list.forEach(function (prod, i) {
      ...
      if (i >= 3 && i < _this4.placeholders.utils.secondCircle.prodsCount + 3) {
        prod.scalingElem.style.transform = "translate(-50%,-50%) scale(1.1)";
      }
      if (i >= _this4.placeholders.utils.secondCircle.prodsCount + 3) {
        prod.scalingElem.style.transform = "translate(-50%,-50%) scale(.6)";
      }
      prod.scalingElem.dataset.elementId = "WowQuiz_Step".concat(parseInt(qIndex) + 2, "-Product");
    });
  ...
  adjustProdPos: function adjustProdPos() {
    var _this5 = this;
    this.prod_list.forEach(function (prod, i) {
      prod.elem.style.transform = "\n      translate(\n        ".concat(_this5.placeholders.coordinates[i].x - prod.x, "px,\n        ").concat(_this5.placeholders.coordinates[i].y - prod.y, "px\n      )");
    });
  },
  setMouseMove: function setMouseMove() {
    var _this6 = this;
    this.prod_list_container.addEventListener('mousemove', function (e) {
      _this6.prod_list_container.style.transform = "translate(".concat(-(e.clientX - window.innerWidth / 2) * .01, "px,").concat(-(e.clientY - window.innerHeight / 2) * .01, "px)");
    });
    document.querySelector('#ct_wow__search__input').addEventListener('mouseenter', function (e) {
      _this6.prod_list_container.style.transform = "translate(0,0)";
    });
  },
  ...
  setCloseHandler: function setCloseHandler() {
    ...
    document.querySelector('#ct_wow__search__close').addEventListener('click', function () {
      _this7.container.classList.remove('ct_in');
      document.body.style.overflow = 'auto';
      //reset structure
      _this7.resetStructure();
      //reset questions
...
window.ct_wow__search.inputManagement = {
  ...
  setButtonsHandler: function setButtonsHandler() {
    ...
    this.buttons.prev.addEventListener('click', function () {
      _this8.changeQuestions('prev');
    });
    this.buttons.next.addEventListener('click', function () {
      _this8.changeQuestions('next');
    });
  ...
  updateProgress: function updateProgress(dir) {
    ...
    if (dir) {
      ...
      if (this.progress.state != 1) {
        (0, _utils.analyticsPush)({
          'id': 'Impression',
          'Page_Section2': "WowQuiz:Step".concat(this.progress.state)
        });
      }
      ...
      setTimeout(function () {
        // start animation
        _this9.progress.current.classList.add('ct_animation');
        _this9.progress.container.style.setProperty('--ct-wow-search-input-progress-after-translate', "translateY(0%)");
        _this9.progress.container.style.setProperty('--ct-wow-search-input-progress-after-opacity', "1");
        if (dir === 'next') {
          _this9.progress.container.style.setProperty('--ct-wow-search-input-progress-before-translate', "translateY(100%)");
        } else {
          _this9.progress.container.style.setProperty('--ct-wow-search-input-progress-before-translate', "translateY(-100%)");
        }
        _this9.progress.container.style.setProperty('--ct-wow-search-input-progress-before-opacity', "0");
      }, 10);
    }
    setTimeout(function () {
      _this9.progress.container.querySelector('.ct_wow__search__input_progress__current span').innerHTML = _this9.progress.state;
      _this9.progress.current.dataset.current = _this9.progress.state;
      _this9.progress.current.querySelector('span').style.opacity = 1;
      _this9.progress.current.classList.remove('ct_animation');
      _this9.blockerActive = false;
    }, 410);
  },
  ...
  updateQuestionCopy: function updateQuestionCopy() {
    ...
    setTimeout(function () {
      _this10.questions.container.querySelector('h3').innerHTML = _storeInfo.storeInfo.getLang(window.ct_wow__search.data.questions[_this10.progress.state - 1].question);
      _this10.questions.container.querySelector('h3').style.opacity = 1;
    }, 400);
  },
  buildAnswers: function buildAnswers() {
    ...
    window.ct_wow__search.data.questions.forEach(function (question, qindex) {
      ...
      question.answers.forEach(function (answer, aindex) {
        answers += "\n        <div class=\"ct_wow__search__button_wrap\">\n          <button class=\"ct_cta ct_cta__black \" data-q=\"".concat(qindex, "\" tabindex=\"-1\" data-a=\"").concat(aindex, "\" data-element-id=\"WowQuiz_Step").concat(qindex + 1, "-Answer\" data-description=\"").concat(answer["en"], "\">").concat(_storeInfo.storeInfo.getLang(answer), "</button>\n        </div>\n        ");
      });
      answers += "</div>";
    });
    ...
    this.answers.container.querySelectorAll('button').forEach(function (button) {
      button.addEventListener('click', function () {
        if (!button.classList.contains('ct_active')) {
          ...
          if (_this11.progress.state < _this11.stepsCount) {
            ...
            _this11.container.classList.add('ct_can_proceed');
            _this11.changeQuestions('next');
          } else {
            _this11.showResult();
          }
        }
  ...
  showResult: function showResult() {
    ...
    (0, _utils.analyticsPush)({
      ...
      'Content_Answers': (0, _utils.formatDataAnalytics)(window.ct_wow__search.inputManagement.answers.tracked, '|'),
      // i valori vanno passati sempre in inglese
      'Products': (0, _utils.formatDataAnalytics)(window.ct_wow__search.inputManagement.results.state, 'object')
    });
    ...
    this.setResult();
    setTimeout(function () {
      _this12.results.container.classList.remove('ct_loader_in');
    }, 2000);
  },
  setResult: function setResult() {
    ...
    this.results.state.forEach(function (upc) {
      productsContainer.innerHTML += "\n      <a href=\"".concat(_storeInfo.storeInfo.getLang(window.ct_wow__search.data.products[upc].url), "\" class=\"ct_wow__search__results_product\" data-element-id=\"WowQuiz_Results-Product\" data-description=\"").concat(upc, "\">\n          <img src=\"").concat(window.ct_wow__search.data.products[upc].img ? window.ct_wow__search.data.products[upc].img : "https://assets.sunglasshut.com/is/image/LuxotticaRetail/" + upc + "__STD__shad__fr.png?impolicy=SGH_bgtransparent&width=640", "\" alt=\"").concat(upc, "\">\n          <span>").concat(window.ct_wow__search.data.products[upc].brand, "</span>\n        </a>\n      ");
    });
  },
  restartQuiz: function restartQuiz() {
    ...
    setTimeout(function () {
      ...
      _this13.resetQuestions();
      _this13.results.container.classList.remove('ct_in');
      _this13.results.container.classList.add('ct_loader_in');
    }, 400);
  },
...
},{"./modules/accessibility":7,"./modules/loader":8,"./modules/storeInfo":9,"./modules/utils":10,"@babel/runtime/helpers/asyncToGenerator":1,"@babel/runtime/helpers/interopRequireDefault":2,"@babel/runtime/regenerator":5}],7:[function(require,module,exports){
"use strict";
...
},{"./storeInfo":9,"./utils":10}],9:[function(require,module,exports){
"use strict";
...
exports.getDevice = exports.formatDataAnalytics = exports.debounce = exports.customLog = exports.checkData = exports.calcCoordinates = exports.analyticsPush = void 0;
...
var checkData = function checkData(data, timeout) {
  return new Promise(function (resolve, reject) {
    if (data.loaded === true) {
      resolve();
    } else {
      var timer = 0;
      var checkDataInterver = setInterval(function () {
        if (data.loaded === true) {
...
var analyticsPush = function analyticsPush(data) {
...
exports.analyticsPush = analyticsPush;
var formatDataAnalytics = function formatDataAnalytics(data, separator) {
  var data2track = {};
  if (separator === 'object') {
...
exports.formatDataAnalytics = formatDataAnalytics;
