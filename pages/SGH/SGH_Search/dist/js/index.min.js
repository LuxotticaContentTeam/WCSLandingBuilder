...
window.ct_wow__search.start = function (elem, section) {
  var this_ = document.querySelector(elem);
  ...
  if (window.ct_wow__search.init === false) {
    ...
    if (window.tealium_data2track) {
      window.tealium_data2track.push({
        'id': 'Click',
        'data_element_id': section + '_WowQuiz_Start',
        'data_description': section === "Search" ? "LaunchQuiz" : "LaunchQuiz_IMG"
      });
    } else {
      console.log({
        'id': 'Click',
...
function ct_wow__search__start() {
  ...
  if (window.ct_wow__search.config.openingElem.length > 0) {
    window.ct_wow__search.config.openingElem.forEach(function (element) {
      if (element.pages === 'all') {
        ...
        element.pages.forEach(function (page) {
