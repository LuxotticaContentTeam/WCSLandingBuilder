...
window.ct_wow__search_structure = {
  ...
  calcHeight: function calcHeight() {
    ...
    window.ct_wow__search_structure.container.style.setProperty("--ct-wow-search-height", "".concat(window.innerHeight, "px"));
  },
  ...
  zoomHandler: function zoomHandler() {
    ...
    function touchMove(event) {
      if (scaling) {
        ...
        if (event.scale > 1) {
          window.ct_wow__search_structure.prod_list_container.style.transform = "scale(".concat(Math.min(1.25, event.scale), ")");
        } else {
          window.ct_wow__search_structure.prod_list_container.style.transform = "scale(".concat(Math.max(1, event.scale), ")");
        }
      }
    }
  ...
  setCloseHandler: function setCloseHandler() {
    ...
    this.container.querySelector('#ct_wow__search__close').addEventListener('click', function () {
      _this5.container.classList.remove('ct_in');
      document.body.style.overflow = 'auto';
      //reset structure
      _this5.resetStructure();
      //reset questions
...
window.ct_wow__search_questions = {
  ...
  setButtonsHandler: function setButtonsHandler() {
    ...
    this.buttons.prev.addEventListener('click', function () {
      _this6.changeQuestions('prev');
    });
    this.buttons.next.addEventListener('click', function () {
      _this6.changeQuestions('next');
    });
  ...
  updateProgress: function updateProgress(dir) {
    ...
    if (dir) {
      ...
      setTimeout(function () {
        // start animation
        _this7.progress.current.classList.add('ct_animation');
        _this7.progress.container.style.setProperty('--ct-wow-search-input-progress-after-translate', "translateY(0%)");
        _this7.progress.container.style.setProperty('--ct-wow-search-input-progress-after-opacity', "1");
        if (dir === 'next') {
          _this7.progress.container.style.setProperty('--ct-wow-search-input-progress-before-translate', "translateY(100%)");
        } else {
          _this7.progress.container.style.setProperty('--ct-wow-search-input-progress-before-translate', "translateY(-100%)");
        }
        _this7.progress.container.style.setProperty('--ct-wow-search-input-progress-before-opacity', "0");
      }, 10);
    }
    setTimeout(function () {
      _this7.progress.container.querySelector('.ct_wow__search__input_progress__current span').innerHTML = _this7.progress.state;
      _this7.progress.current.dataset.current = _this7.progress.state;
      _this7.progress.current.querySelector('span').style.opacity = 1;
      _this7.progress.current.classList.remove('ct_animation');
      _this7.blockerActive = false;
    }, 410);
  },
  updateQuestionCopy: function updateQuestionCopy() {
    ...
    setTimeout(function () {
      _this8.questions.container.querySelector('h3').innerHTML = window.ct_wow__search__data_question[_this8.progress.state - 1].question["en"];
      _this8.questions.container.querySelector('h3').style.opacity = 1;
    }, 400);
  },
  buildAnswers: function buildAnswers() {
    var answers = "";
    window.ct_wow__search__data_question.forEach(function (question, i) {
      answers += "<div class=\"ct_wow__search__input_answer\" data-answer=\"".concat(i, "\">");
      question.answers.forEach(function (answer) {
        answers += "\n        <div class=\"ct_wow__search__button_wrap\">\n          <button class=\"ct_cta ct_cta__white \">".concat(answer["en"], "</button>\n        </div>\n        ");
      });
      answers += "</div>";
    });
    ...
    this.answers.container.querySelectorAll('button').forEach(function (button) {
      button.addEventListener('click', function () {
        if (!button.classList.contains('ct_active')) {
          ...
          _this9.container.classList.add('ct_can_proceed');
          if (_this9.progress.state < 5) {
            _this9.changeQuestions('next');
          } else {
            _this9.buttons.results.classList.remove('ct_disabled');
          }
        }
      });
    });
  ...
  showResult: function showResult() {
    ...
    this.setResult();
    setTimeout(function () {
      _this10.results.container.querySelector('#ct_wow__search__results_loader').classList.remove('ct_in');
      _this10.results.container.classList.remove('ct_loader_in');
    }, 2000);
  },
  setResult: function setResult() {
    ...
    resultsProd.forEach(function (prod) {
      productsContainer.innerHTML += "\n        <a href=\"/".concat(prod.prodId, "\" class=\"ct_wow__search__results_product\">\n          <img src=\"").concat(prod.prodImg, "\" />\n          <span>").concat(prod.brandName, "</span>\n        </a>\n      ");
    });
  },
  restartQuiz: function restartQuiz() {
    var _this11 = this;
    _loader.loader.init(true, 1000);
    setTimeout(function () {
      window.ct_wow__search_structure.resetStructure();
      _this11.resetQuestions();
      _this11.results.container.classList.remove('ct_in');
      _this11.results.container.classList.add('ct_loader_in');
    }, 400);
  },
...
},{"./modules/loader":16,"./modules/storeInfo":17,"./modules/utils":18,"@babel/runtime/helpers/asyncToGenerator":3,"@babel/runtime/helpers/defineProperty":4,"@babel/runtime/helpers/interopRequireDefault":5,"@babel/runtime/helpers/toConsumableArray":9,"@babel/runtime/regenerator":14}],16:[function(require,module,exports){
"use strict";
...
var loader = {
  elem: null,
  time: 2400,
  init: function init(loader, time) {
    var _this = this;
    ...
    if (loader) {
      ...
      setTimeout(function () {
        ...
        _this.elem.classList.remove('ct_in');
        (0, _utils.customLog)('loader out 1');
      }, time ? time : this.time);
    } else {
      setTimeout(function () {
        document.dispatchEvent(loaderEvent);
        (0, _utils.customLog)('loader out 2');
      }, 20);
    }
  },
  loaderAnim: function loaderAnim() {
    var _this2 = this;
    var count = 1;
    var currentId;
    var loaderInvterval = setInterval(function () {
      if (!_this2.elem.querySelector('use.ct_active')) {
        _this2.elem.querySelector('use:nth-child(1)').classList.add('ct_active');
      } else {
        _this2.elem.querySelector('use.ct_active').classList.remove('ct_active');
        if (currentId < 5) {
          currentId += 1;
        } else {
          currentId = 1;
        }
        _this2.elem.querySelector("use:nth-child(".concat(currentId, ")")).classList.add('ct_active');
      }
      count += 300;
      if (count >= _this2.time * 2) {
