...
function ct_build__controls(position,volume,playing){
    var ct_video__controls = '\
        <div class="ct_video__controls ct_'+(position?position:"right")+' ">'+
            '<button class="ct_play_pause'+(playing?"ct_playing":"")+'">'+
                ct_icons.play +
                ct_icons.pause +
            '</button>'+
            '<button class="ct_volume_on_off ct_muted '+ (volume?"":"ct_disabled" )+'">'+
                ct_icons.mute+
                ct_icons.volume+
            '</button>'+
        '</div>';
...
function ct_scroll_to_seciton(elem,offset){
    ...
    let sectionTop = document.querySelector(elem).getBoundingClientRect().top 
    window.scroll({
        behavior: 'smooth',
        left: 0,
...
function ct_accordion(button){
    let accordion_wrap = button.parentNode.querySelector('.ct_accordion__content_wrap');
    let accordion_content = accordion_wrap.querySelector('.ct_accordion__content');
    let accordion_icon = button.parentNode.querySelector('.ct_accordion__icon');
    if (accordion_wrap.clientHeight === 0){
        accordion_wrap.style.height = `${accordion_content.clientHeight}px`;
        if (accordion_icon){
